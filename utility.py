...
def build_search_pipeline(query, allowed_ids, skip, limit):
    ...
    sort_stage = {"$sort": {"score": -1, "file_name": 1,  "_id": 1}}  # Sort *before* pagination
...
async def upsert_tmdb_info(tmdb_id, tmdb_type):
    """
...
async def process_tmdb_info(bot, file_info):
    ...
    if file_info["channel_id"] not in TMDB_CHANNEL_ID:
        return  # Not a TMDB channel, so TMDB processing is not applicable
